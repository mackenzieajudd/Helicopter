...
int main()
{
	...
	//Initialisation for VGA and start menu
	...
	while(1 == 1)
	{
		...
		//Timer used for FPS regulation
		alt_timestamp_start();

		//Loops while game is in progress
		while(1 == 1)
		{
			fps = 0;
			...
			if((float)(alt_timestamp())/(float)(alt_timestamp_freq()) > (float)1/(float)FPS)
			{
				...
				//Carries out collision animatoins once a collision is detected
...
void audio_isr(void * context, unsigned int irq_id)
{
	if (flag1 == 1)
	{
		...
		alt_up_audio_write_fifo(audio, &buf1[k1], SAMPLE_SIZE, ALT_UP_AUDIO_LEFT);
		alt_up_audio_write_fifo(audio, &buf1[k1], SAMPLE_SIZE, ALT_UP_AUDIO_RIGHT);
	...
	else if (flag == 1)
	{
		...
		alt_up_audio_write_fifo(audio, &buf2[k2], SAMPLE_SIZE, ALT_UP_AUDIO_LEFT);
		alt_up_audio_write_fifo(audio, &buf2[k2], SAMPLE_SIZE, ALT_UP_AUDIO_RIGHT);
	...
	if (k3 >= (SONG3_SIZE)/2)
		...
		alt_up_audio_write_fifo(audio, &buf3[k3], SAMPLE_SIZE, ALT_UP_AUDIO_LEFT);
		alt_up_audio_write_fifo(audio, &buf3[k3], SAMPLE_SIZE, ALT_UP_AUDIO_RIGHT);
		k3 = k3 + 96;
	}
}
